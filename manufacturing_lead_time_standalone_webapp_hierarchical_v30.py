...
function renderNetwork(data,shouldFit=true){
  ...
  const targetAssignments={}; // key "from__to" -> {side, slot, countOnSide, order}
  Object.entries(incomingByTarget).forEach(([target, arr])=>{
    ...
    // for >1 incoming: 2 => top,bottom ; 3 => top,left,bottom ; >3 cycle top,left,bottom with per-side slots
    const sideCounters={top:0,left:0,bottom:0};
    const sidePattern = arr.length===2 ? ['top','bottom'] : ['top','left','bottom'];
    arr.forEach((edge,idx)=>{
      const side = sidePattern[idx % sidePattern.length];
      const slot = sideCounters[side]++;
      targetAssignments[`${edge.from}__${edge.to}`]={side, slot};
    });
  });
  ...
  function routeEdge(edge){
    ...
    if(assign.side==='left'){
      // Keep each incoming connector distinct until target: dedicated approach column per edge
      const approachX = end.x - targetBuffer - assign.slot*slotGap;
      const midX = Math.max(start.x + sourceBuffer, approachX);
      pts.push({x: midX, y: start.y});
      pts.push({x: midX, y: end.y});
      pts.push(end);
    } else if(assign.side==='top'){
      // Dedicated horizontal approach lane above target
      const approachY = end.y - targetBuffer - assign.slot*slotGap;
      const bendX = Math.max(start.x + sourceBuffer, end.x - 26 + assign.slot*12);
