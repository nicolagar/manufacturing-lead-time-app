...
function renderNetwork(data,shouldFit=true){
  ...
  const cardW=176, cardH=74, colGap=170, laneGap=84, marginX=60, marginY=46;
  const positions={};
  let maxX=0,maxY=0;
  topRows.forEach(r=>{
  ...
  networkViewport.contentW=Math.max(maxX+100,1280);
  networkViewport.contentH=Math.max(maxY+100,460);
  ...
  // Determine incoming-anchor assignments per target according to locked rules:
  // 1 incoming -> left
  // >1 incoming -> distribute across top/left/bottom (never right)
  const incomingByTarget={};
  topEdges.forEach(e=>{
    (incomingByTarget[e.to]||(incomingByTarget[e.to]=[])).push(e);
  });

  // Geometry-aware side assignment:
  // 1 incoming => left
  // >1 incoming => choose top/left/bottom from source position relative to target
  // never right for incoming anchors
  const targetAssignments={};
  Object.entries(incomingByTarget).forEach(([target, arr])=>{
    const tgt=positions[target];
    const topThreshold=tgt.y - 24;
    const bottomThreshold=tgt.y + tgt.h + 24;

    arr.sort((a,b)=>{
      const aCrit=dominantEdges.has(`${a.from}__${a.to}`)?-1:0;
      const bCrit=dominantEdges.has(`${b.from}__${b.to}`)?-1:0;
      if(aCrit!==bCrit) return aCrit-bCrit;
      const ay=(positions[a.from]?.y ?? 0) + (positions[a.from]?.h ?? 0)/2;
      const by=(positions[b.from]?.y ?? 0) + (positions[b.from]?.h ?? 0)/2;
      return ay-by;
    });

    if(arr.length===1){
      targetAssignments[`${arr[0].from}__${arr[0].to}`]={side:'left', slot:0};
      return;
    }

    const sideCounters={top:0,left:0,bottom:0};
    const reservedSides=new Set();

    function preferredSide(edge){
      const src=positions[edge.from];
      const srcCy=src.y + src.h/2;
      if(srcCy < topThreshold) return 'top';
      if(srcCy > bottomThreshold) return 'bottom';
      return 'left';
    }

    arr.forEach(edge=>{
      const key=`${edge.from}__${edge.to}`;
      const pref=preferredSide(edge);
      if(!reservedSides.has(pref)){
        targetAssignments[key]={side:pref, slot:sideCounters[pref]++};
        reservedSides.add(pref);
      }
    });

    arr.forEach(edge=>{
      const key=`${edge.from}__${edge.to}`;
      if(targetAssignments[key]) return;
      const pref=preferredSide(edge);
      const fallbackOrder =
        pref==='top' ? ['left','bottom','top'] :
        pref==='bottom' ? ['left','top','bottom'] :
        ['top','bottom','left'];
      const side = fallbackOrder.find(s=>!reservedSides.has(s)) || 'left';
      targetAssignments[key]={side, slot:sideCounters[side]++};
  ...
  function routeEdge(edge){
    ...
    if(!from || !to) return [];
    const assign=targetAssignments[`${edge.from}__${edge.to}`] || {side:'left', slot:0};

    const start=rightMid(from); // outgoing always right
    let end;
    if(assign.side==='left') end=leftMid(to);
    else if(assign.side==='top') end=topMid(to);
    else end=bottomMid(to);

    const sourceBuffer=26;
    const targetBuffer=34;
    const slotGap=18;
    const minApproachRun=44;

    let pts=[start];

    if(assign.side==='left'){
      const approachX=end.x - targetBuffer - assign.slot*slotGap;
      const bendX=Math.max(start.x + sourceBuffer, approachX - minApproachRun);
      pts.push({x:bendX, y:start.y});
      pts.push({x:approachX, y:start.y});
      pts.push({x:approachX, y:end.y});
      pts.push(end);
    } else if(assign.side==='top'){
      const approachY=end.y - targetBuffer - assign.slot*slotGap;
      const bendX=Math.max(start.x + sourceBuffer, end.x - minApproachRun + assign.slot*8);
      pts.push({x:bendX, y:start.y});
  ...
  function cardMarkup(node,row,isDominant){
    ...
    return `<rect x="${node.x}" y="${node.y}" width="${node.w}" height="${node.h}" rx="8" fill="#ffffff" stroke="${stroke}" stroke-width="${strokeW}" />`+
      `<text x="${node.x+node.w/2}" y="${node.y+23}" text-anchor="middle" font-size="13" fill="#111827">${esc(row.process)}</text>`+
      `<text x="${node.x+10}" y="${node.y+49}" font-size="10" fill="#64748b">Start ${es}</text>`+
      `<text x="${node.x+node.w/2}" y="${node.y+49}" text-anchor="middle" font-size="10" fill="#64748b">Dur ${dur}</text>`+
      `<text x="${node.x+node.w-10}" y="${node.y+49}" text-anchor="end" font-size="10" fill="#64748b">End ${ef}</text>`;
  }
  ...
  // Draw non-critical edges first, then critical edges on top (foreground)
