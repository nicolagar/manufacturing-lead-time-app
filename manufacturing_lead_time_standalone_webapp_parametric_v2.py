...
body{margin:0;font-family:Arial,sans-serif;background:#f8fafc;color:#0f172a}.wrap{max-width:1480px;margin:0 auto;padding:24px}.grid-top{display:grid;grid-template-columns:1.6fr 1fr;gap:16px;margin-bottom:16px}.card{background:#fff;border:1px solid #e2e8f0;border-radius:20px;padding:20px}.actions{display:flex;flex-wrap:wrap;gap:10px;margin-top:16px}button,.file-label{border:1px solid #e2e8f0;background:#fff;border-radius:14px;padding:10px 14px;cursor:pointer}button.primary{background:#2563eb;color:#fff;border-color:#2563eb}.file-label input{display:none}.badge{display:inline-block;background:#eef2ff;color:#3730a3;padding:6px 10px;border-radius:999px;font-size:12px}.status-ok{border-radius:16px;padding:14px;margin-top:8px;background:#d1fae5;color:#065f46}.status-warn{border-radius:16px;padding:14px;margin-top:8px;background:#fef3c7;color:#92400e}.stats{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px}.stat{background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:12px}.stat-label{color:#64748b;font-size:12px;text-transform:uppercase}th{text-align:left;color:#64748b;font-size:13px;padding:0 8px;white-space:nowrap}td{padding:0 8px}table{width:100%;border-collapse:separate;border-spacing:0 10px}input[type=text],input[type=number]{width:100%;border:1px solid #e2e8f0;border-radius:12px;padding:10px 12px;font-size:14px}.section{margin-top:16px}.scroll{overflow-x:auto}.chart-box{overflow-x:auto;border:1px solid #e2e8f0;border-radius:16px;background:#fff;padding:10px}@media (max-width:900px){.grid-top{grid-template-columns:1fr}.stats{grid-template-columns:1fr}}</style><script src='https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js'></script></head><body>
<div class='wrap'><div class='grid-top'><div class='card'><h1>Manufacturing Lead Time Calculator</h1><p>Duration may be numeric or a formula token such as <b>Lead_A</b>. If duration is text, the app looks in sheet <b>FORMULA</b> column 1, takes the equation from column 2, and resolves variables from the same PERT row starting at column 5.</p><div class='actions'><button onclick='downloadTemplate()'>Download template</button><button onclick='downloadCurrentInput()'>Download current input</button><label class='file-label'>Upload Excel<input type='file' accept='.xlsx,.xls' onchange='uploadExcel(event)'></label><button onclick='addRow()'>Add row</button><button class='primary' onclick='runCalculation()'>Run</button></div><div style='margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;'><span id='loadedFile' class='badge' style='display:none;'></span><span class='badge'>Sheets: PERT + FORMULA</span></div></div><div class='card'><h2>Summary</h2><div id='summary'></div></div></div>
<div class='card section'><h2>Input table</h2><div class='scroll'><table><thead><tr><th>Process</th><th>Predecessor</th><th>Successor</th><th>Duration / Token</th><th>unitary_value</th><th>additional</th><th>workers</th><th>p0</th><th>p1</th><th></th></tr></thead><tbody id='rowsBody'></tbody></table></div></div>
<div class='card section'><h2>Formula table</h2><div class='scroll'><table><thead><tr><th>Name</th><th>Formula</th><th></th></tr></thead><tbody id='formulaBody'></tbody></table></div></div>
<div class='card section'><h2>Gantt chart</h2><div id='ganttContainer' class='chart-box'></div></div><div class='card section'><h2>Network diagram</h2><div id='networkContainer' class='chart-box'></div></div><div class='card section'><h2>Calculated schedule</h2><div class='scroll'><table id='scheduleTable'></table></div></div></div>
<script>
const sampleRows=%SAMPLE_ROWS_JSON%;const sampleFormulas=%SAMPLE_FORMULAS_JSON%;let rows=structuredClone(sampleRows);let formulas=structuredClone(sampleFormulas);function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}function renderRows(){document.getElementById('rowsBody').innerHTML=rows.map((r,i)=>`<tr><td><input type='text' value='${esc(r.process)}' oninput="updateRow(${i},'process',this.value)"></td><td><input type='text' value='${esc(r.predecessor)}' oninput="updateRow(${i},'predecessor',this.value)"></td><td><input type='text' value='${esc(r.successor)}' oninput="updateRow(${i},'successor',this.value)"></td><td><input type='text' value='${esc(r.duration)}' oninput="updateRow(${i},'duration',this.value)"></td><td><input type='number' step='any' value='${esc(r.unitary_value)}' oninput="updateRow(${i},'unitary_value',this.value)"></td><td><input type='number' step='any' value='${esc(r.additional)}' oninput="updateRow(${i},'additional',this.value)"></td><td><input type='number' step='any' value='${esc(r.workers)}' oninput="updateRow(${i},'workers',this.value)"></td><td><input type='number' step='any' value='${esc(r.p0)}' oninput="updateRow(${i},'p0',this.value)"></td><td><input type='number' step='any' value='${esc(r.p1)}' oninput="updateRow(${i},'p1',this.value)"></td><td><button onclick='removeRow(${i})'>Delete</button></td></tr>`).join('');}
function renderFormulas(){document.getElementById('formulaBody').innerHTML=formulas.map((r,i)=>`<tr><td><input type='text' value='${esc(r.name)}' oninput="updateFormula(${i},'name',this.value)"></td><td><input type='text' value='${esc(r.formula)}' oninput="updateFormula(${i},'formula',this.value)"></td><td><button onclick='removeFormula(${i})'>Delete</button></td></tr>`).join('')+`<tr><td colspan='3'><button onclick='addFormula()'>Add formula</button></td></tr>`;}
function updateRow(i,f,v){rows[i][f]=v;}function addRow(){rows.push({process:'',predecessor:'NA',successor:'STOP',duration:'',unitary_value:'',additional:'',workers:'',p0:'',p1:''});renderRows();}function removeRow(i){rows.splice(i,1);if(!rows.length)addRow();else renderRows();}function updateFormula(i,f,v){formulas[i][f]=v;}function addFormula(){formulas.push({name:'',formula:''});renderFormulas();}function removeFormula(i){formulas.splice(i,1);renderFormulas();}
function rowsForApi(){return rows.filter(r=>r.process||r.predecessor||r.successor||r.duration!=='').map(r=>({process:String(r.process||'').trim(),predecessor:String(r.predecessor||'NA').trim()||'NA',successor:String(r.successor||'STOP').trim()||'STOP',duration:String(r.duration??'').trim(),unitary_value:r.unitary_value,additional:r.additional,workers:r.workers,p0:r.p0,p1:r.p1}));}
...
function downloadCurrentInput(){const wsPert=XLSX.utils.json_to_sheet(rows,{header:['process','predecessor','successor','duration','unitary_value','additional','workers','p0','p1']});const wsFormula=XLSX.utils.json_to_sheet(formulas,{header:['name','formula']});const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,wsPert,'PERT');XLSX.utils.book_append_sheet(wb,wsFormula,'FORMULA');XLSX.writeFile(wb,'PERT_input.xlsx');}
async function uploadExcel(event){const file=event.target.files?.[0];if(!file)return;const loaded=document.getElementById('loadedFile');loaded.style.display='inline-block';loaded.textContent='Loaded: '+file.name;const data=await file.arrayBuffer();const wb=XLSX.read(data,{type:'array'});const pert=wb.Sheets.PERT||wb.Sheets[wb.SheetNames[0]];const rawPert=XLSX.utils.sheet_to_json(pert,{header:1,defval:''});const first=(rawPert[0]||[]).slice(0,4).map(x=>String(x).trim().toLowerCase()).sort().join('|');const body=first===['process','predecessor','successor','duration'].sort().join('|')?rawPert.slice(1):rawPert;rows=body.filter(r=>r.some(cell=>String(cell??'').trim()!=='' )).map(r=>({process:String(r[0]??'').trim(),predecessor:String(r[1]??'NA').trim()||'NA',successor:String(r[2]??'STOP').trim()||'STOP',duration:String(r[3]??'').trim(),unitary_value:String(r[4]??'').trim(),additional:String(r[5]??'').trim(),workers:String(r[6]??'').trim(),p0:String(r[7]??'').trim(),p1:String(r[8]??'').trim()}));if(!rows.length)rows=structuredClone(sampleRows);const formula=wb.Sheets.FORMULA;if(formula){const rawFormula=XLSX.utils.sheet_to_json(formula,{header:1,defval:''});formulas=rawFormula.slice(1).filter(r=>String(r[0]??'').trim()&&String(r[1]??'').trim()).map(r=>({name:String(r[0]??'').trim(),formula:String(r[1]??'').trim()}));}if(!formulas.length)formulas=structuredClone(sampleFormulas);renderRows();renderFormulas();}
...
renderRows();renderFormulas();document.getElementById('summary').innerHTML='<div class="status-warn">Press Run to calculate.</div>';
</script></body></html>"""
