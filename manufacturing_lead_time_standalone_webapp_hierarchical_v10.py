...
function renderGantt(data){const schedule=(data.schedule||[]).slice().sort((a,b)=>a.depth-b.depth||a.earliest_start-b.earliest_start||a.process.localeCompare(b.process));const dominantSet=new Set(data.dominant_path||[]);const width=1320,rowHeight=42,labelWidth=240,rightPad=24,topPad=26,chartWidth=width-labelWidth-rightPad,height=topPad+schedule.length*rowHeight+30,finish=Math.max(data.lead_time||1,1),scale=value=>labelWidth+(value/finish)*chartWidth,fmt=value=>Math.round(value*1000)/1000;let svg=`<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">`;const tickCount=Math.min(Math.round(finish)+1,21);for(let i=0;i<tickCount;i++){const tick=(finish/Math.max(tickCount-1,1))*i,x=scale(tick);svg+=`<line x1="${x}" y1="16" x2="${x}" y2="${height-16}" stroke="#e5e7eb" stroke-width="1" /><text x="${x}" y="12" text-anchor="middle" font-size="11" fill="#6b7280">${fmt(tick)}</text>`;}schedule.forEach((row,idx)=>{const y=topPad+idx*rowHeight,x=scale(row.earliest_start),w=Math.max(scale(row.earliest_finish)-x,6),hue=(idx*37)%360,fill=`hsl(${hue} 70% 65%)`,isDominant=dominantSet.has(row.process)&&row.depth===0,stroke=isDominant?'#dc2626':row.critical?'#111827':'transparent',sw=isDominant?3:row.critical?2:0,label=`${'— '.repeat(row.depth)}${row.process}`,details=`S:${fmt(row.earliest_start)} | D:${fmt(row.duration)} | E:${fmt(row.earliest_finish)}`;svg+=`<text x="10" y="${y+15}" font-size="12" fill="#111827">${esc(label)}</text><text x="10" y="${y+29}" font-size="10" fill="#475569">${esc(details)}</text><rect x="${x}" y="${y+4}" width="${w}" height="20" rx="8" fill="${fill}" stroke="${stroke}" stroke-width="${sw}" /><text x="${Math.min(x+6, x+w-4)}" y="${y+18}" font-size="10" fill="#111827">${esc(details)}</text>`;});svg+='</svg>';document.getElementById('ganttContainer').innerHTML=svg;}
...
function toggleParent(process){expandedParents[process]=!expandedParents[process];if(window.__lastNetworkData){renderNetwork(window.__lastNetworkData);}}
function renderNetwork(data){
  window.__lastNetworkData=data;
  const graph=data.graph||{nodes:[],edges:[]};
  const topLevel=(data.schedule||[]).filter(r=>r.depth===0).sort((a,b)=>a.earliest_start-b.earliest_start||a.process.localeCompare(b.process));
  const byRefines={};
  (data.schedule||[]).forEach(r=>{const parent=r.refines||'';(byRefines[parent]||(byRefines[parent]=[])).push(r);});
  Object.values(byRefines).forEach(arr=>arr.sort((a,b)=>a.earliest_start-b.earliest_start||a.process.localeCompare(b.process)));

  const topY=50, topH=46, topW=128, topGap=72;
  const childW=104, childH=38, childGapX=24, childGapY=18, innerPad=16, parentPadBottom=18;
  const marginX=40, marginY=30;
  const positions={}, parentBoxes={}, childToParent={}, edges=(graph.edges||[]);
  let cursorX=marginX;
  let maxBottom=0;

  topLevel.forEach(row=>{
    const proc=row.process;
    const children=(byRefines[proc]||[]).filter(c=>c.process!==proc);
    const expanded=!!expandedParents[proc];
    const x=cursorX, y=topY;

    positions[proc]={x,y,w:topW,h:topH,depth:0};
    let box={x:x-12,y:y-12,w:topW+24,h:topH+24,expanded:false,children:children.map(c=>c.process)};
    let advance=topW+topGap;

    if(children.length && expanded){
      const cols=Math.max(1, Math.ceil(Math.sqrt(children.length)));
      const rows=Math.ceil(children.length/cols);
      const innerW=cols*childW+(cols-1)*childGapX;
      const innerH=rows*childH+(rows-1)*childGapY;
      const boxW=Math.max(topW+24, innerPad*2+innerW);
      const boxH=topH+20+innerPad+innerH+parentPadBottom;
      box={x:x-14,y:y-16,w:boxW,h:boxH,expanded:true,children:children.map(c=>c.process)};
      const childStartX=box.x+innerPad;
      const childStartY=y+topH+22;
      children.forEach((child, idx)=>{
        const col=idx%cols, rowIdx=Math.floor(idx/cols);
        const cx=childStartX+col*(childW+childGapX);
        const cy=childStartY+rowIdx*(childH+childGapY);
        positions[child.process]={x:cx,y:cy,w:childW,h:childH,depth:child.depth||1};
        childToParent[child.process]=proc;
      });
      advance=boxW+topGap;
    ...
    parentBoxes[proc]=box;
    cursorX += advance;
  });

  // Position any remaining non-top nodes not yet positioned
  let extraY=maxBottom+40;
  (data.schedule||[]).filter(r=>r.depth>0 && !positions[r.process]).forEach((row,idx)=>{
    positions[row.process]={x:marginX+(idx%4)*(childW+40), y:extraY+Math.floor(idx/4)*(childH+24), w:childW, h:childH, depth:row.depth};
    maxBottom=Math.max(maxBottom, positions[row.process].y+childH);
  });

  const width=Math.max(cursorX+marginX, 1200);
  const height=Math.max(maxBottom+marginY, 260);

  function rightMid(p){return {x:p.x+p.w, y:p.y+p.h/2};}
  function leftMid(p){return {x:p.x, y:p.y+p.h/2};}
  function boxContainsPoint(box, pt){return pt.x>box.x && pt.x<box.x+box.w && pt.y>box.y && pt.y<box.y+box.h;}
  function escapeObstacleX(startX, endX, y, sourceBox, targetBox){
    let bendX=(startX+endX)/2;
    const obstacles=Object.values(parentBoxes);
    for(const box of obstacles){
      if(box===sourceBox||box===targetBox) continue;
      const overlapY = y>box.y-8 && y<box.y+box.h+8;
      const overlapX = bendX>box.x-12 && bendX<box.x+box.w+12;
      if(overlapY && overlapX){
        bendX = box.x+box.w+20;
      }
    }
    return bendX;
  }

  let svg=`<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><defs><marker id="arrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,6 L9,3 z" fill="#64748b" /></marker><marker id="arrowRed" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,6 L9,3 z" fill="#dc2626" /></marker></defs>`;

  // Draw parent containers first
  Object.entries(parentBoxes).forEach(([proc, box], idx)=>{
    const hasChildren=(box.children||[]).length>0;
    const isExpanded=!!box.expanded;
    const fill='rgba(148,163,184,0.06)';
    svg += `<rect x="${box.x}" y="${box.y}" width="${box.w}" height="${box.h}" rx="18" fill="${fill}" stroke="#cbd5e1" stroke-width="1.5" />`;
    if(hasChildren){
      const bx=box.x+box.w-26, by=box.y+10;
      svg += `<g onclick="toggleParent('${proc}')" style="cursor:pointer"><rect x="${bx}" y="${by}" width="18" height="18" rx="4" fill="#ffffff" stroke="#94a3b8" stroke-width="1.2" /><text x="${bx+9}" y="${by+13}" text-anchor="middle" font-size="14" fill="#334155">${isExpanded?'-':'+'}</text></g>`;
    }
  });

  const topDominant=new Set(data.dominant_path||[]);
  const dominantEdgeSet=new Set((data.critical_edges||[]).map(e=>`${e.from}__${e.to}`));
  ...
  edges.forEach(e=>{
    ...
    const sourceParent=childToParent[e.from] ? parentBoxes[childToParent[e.from]] : parentBoxes[e.from];
    const targetParent=childToParent[e.to] ? parentBoxes[childToParent[e.to]] : parentBoxes[e.to];

    let p1=rightMid(from), p4=leftMid(to);

    // Keep connectors outside the node/box bodies
    p1.x += 4;
    p4.x -= 4;

    const sameParent = childToParent[e.from] && childToParent[e.from]===childToParent[e.to];
    let bendX;
    if(sameParent){
      bendX=Math.max(p1.x+14, p4.x-14);
      if(bendX>=p4.x) bendX=(p1.x+p4.x)/2;
    } else {
      bendX=escapeObstacleX(p1.x+18, p4.x-18, p1.y, sourceParent, targetParent);
      if(bendX<=p1.x+8) bendX=p1.x+24;
      if(bendX>=p4.x-8) bendX=(p1.x+p4.x)/2;
    }

    const d=`M ${p1.x} ${p1.y} L ${bendX} ${p1.y} L ${bendX} ${p4.y} L ${p4.x} ${p4.y}`;
    const isDom=dominantEdgeSet.has(`${e.from}__${e.to}`);
    svg += `<path d="${d}" fill="none" stroke="${isDom?'#dc2626':'#94a3b8'}" stroke-width="${isDom?3:1.8}" marker-end="url(#${isDom?'arrowRed':'arrow'})" stroke-linejoin="round" />`;
  });

  // Draw nodes on top
  Object.entries(positions).forEach(([id, p], idx)=>{
    const hue=(idx*37)%360;
    const fill=`hsl(${hue} 70% 92%)`;
    const isTop=p.depth===0;
    const isDominant=isTop && topDominant.has(id);
    svg += `<rect x="${p.x}" y="${p.y}" width="${p.w}" height="${p.h}" rx="${isTop?12:10}" fill="${fill}" stroke="${isDominant?'#dc2626':'#334155'}" stroke-width="${isDominant?2.8:1.5}" />`;
    svg += `<text x="${p.x+p.w/2}" y="${p.y+p.h/2+4}" text-anchor="middle" font-size="${isTop?13:12}" fill="#111827">${esc(id)}</text>`;
  });

  svg += `</svg>`;
  document.getElementById('networkContainer').innerHTML=svg;
}
function renderScheduleTable(data){const schedule=data.schedule||[],table=document.getElementById('scheduleTable');let html=`<thead><tr><th>Process</th><th>Refines</th><th>Depth</th><th>Duration token</th><th>Resolved duration</th><th>ES</th><th>EF</th><th>LS</th><th>LF</th><th>Float</th><th>Critical</th></tr></thead><tbody>`;schedule.forEach(r=>{html+=`<tr><td>${esc(r.process)}</td><td>${esc(r.refines)}</td><td>${r.depth}</td><td>${esc(r.duration_raw)}</td><td>${Math.round(r.duration*1000)/1000}</td><td>${Math.round(r.earliest_start*1000)/1000}</td><td>${Math.round(r.earliest_finish*1000)/1000}</td><td>${Math.round(r.latest_start*1000)/1000}</td><td>${Math.round(r.latest_finish*1000)/1000}</td><td>${Math.round(r.total_float*1000)/1000}</td><td>${r.critical?'Yes':'No'}</td></tr>`;});html+='</tbody>';table.innerHTML=html;}
