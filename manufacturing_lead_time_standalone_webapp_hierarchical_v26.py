...
function renderNetwork(data,shouldFit=true){
  ...
  const cardW=170, cardH=72, colGap=150, laneGap=70, marginX=50, marginY=40;
  const positions={};
  let maxX=0,maxY=0;
  topRows.forEach(r=>{
  ...
  networkViewport.contentW=Math.max(maxX+80,1200);
  networkViewport.contentH=Math.max(maxY+80,420);
  ...
  function routeEdge(fromId,toId){
    ...
    if(end.x >= start.x + 36){
      ...
      const dogX1=start.x+32, dogX2=end.x-32, midY=(start.y+end.y)/2;
      pts.push({x:dogX1,y:start.y},{x:dogX1,y:midY},{x:dogX2,y:midY},{x:dogX2,y:end.y});
    }
    pts.push(end);
  ...
  function cardMarkup(node,row,isDominant){
    ...
    return `<rect x="${node.x}" y="${node.y}" width="${node.w}" height="${node.h}" rx="8" fill="#ffffff" stroke="${stroke}" stroke-width="${strokeW}" />`+
      ...
      `<text x="${node.x+10}" y="${node.y+48}" font-size="10" fill="#64748b">Start ${es}</text>`+
      `<text x="${node.x+node.w/2}" y="${node.y+48}" text-anchor="middle" font-size="10" fill="#64748b">Dur ${dur}</text>`+
      `<text x="${node.x+node.w-10}" y="${node.y+48}" text-anchor="end" font-size="10" fill="#64748b">End ${ef}</text>`;
  }
  ...
  topEdges.forEach(e=>{
    const pts=routeEdge(e.from,e.to);
    if(!pts) return;
    const isDom=dominantEdges.has(`${e.from}__${e.to}`);
    svg+=drawOrthoPath(pts,isDom?'#dc2626':'#6b7c93',isDom?2.8:1.9,isDom?'arrowRed':'arrow');
  });
  ...
  renderNestedNetworks(data, byParent, scheduleMap);
}
function renderNestedNetworks(data, byParent=null, scheduleMap=null){
  const host=document.getElementById('nestedNetworkContainer');
  if(!host) return;
  const schedule=data.schedule||[];
  if(!byParent) byParent=groupRowsByParent(schedule);
  if(!scheduleMap){scheduleMap={}; schedule.forEach(r=>scheduleMap[r.process]=r);}
  const graph=data.graph||{edges:[]};

  function renderOne(parent){
    const rows=(byParent[parent]||[]).filter(r=>r.process!==parent);
    if(!rows.length) return '';
    const edges=(graph.edges||[]).filter(e=>(scheduleMap[e.from]?.refines||'')===parent && (scheduleMap[e.to]?.refines||'')===parent);
    const levels=computeLevels(rows, edges);
    const lanes=computeLanes(rows, edges, levels);
    const cardW=140, cardH=62, colGap=120, laneGap=60, marginX=30, marginY=30;
    const positions={};
    let maxX=0,maxY=0;
    rows.forEach(r=>{
      const x=marginX+(levels[r.process]||0)*(cardW+colGap);
    ...
    function routeEdge(fromId,toId){
      ...
      const dx=(to.x+to.w/2)-(from.x+from.w/2), dy=(to.y+to.h/2)-(from.y+from.h/2);
      let start,end;
      if(Math.abs(dx)>=Math.abs(dy)){ start=dx>=0?rightMid(from):leftMid(from); end=dx>=0?leftMid(to):rightMid(to); }
      else { start=dy>=0?bottomMid(from):topMid(from); end=dy>=0?topMid(to):bottomMid(to); }
      let pts=[start];
      if(end.x >= start.x + 36){ const midX=(start.x+end.x)/2; pts.push({x:midX,y:start.y},{x:midX,y:end.y}); }
      else if(end.y >= start.y + 36){ const midY=(start.y+end.y)/2; pts.push({x:start.x,y:midY},{x:end.x,y:midY}); }
      else { const dogX1=start.x+30, dogX2=end.x-30, midY=(start.y+end.y)/2; pts.push({x:dogX1,y:start.y},{x:dogX1,y:midY},{x:dogX2,y:midY},{x:dogX2,y:end.y}); }
      pts.push(end);
      return simplifyPoints(pts);
    }
    function cardMarkup(node,row){
      const es=Math.round((row.earliest_start||0)*1000)/1000;
      const ef=Math.round((row.earliest_finish||0)*1000)/1000;
      return `<rect x="${node.x}" y="${node.y}" width="${node.w}" height="${node.h}" rx="8" fill="#ffffff" stroke="#334155" stroke-width="1.1" />`+
        `<text x="${node.x+node.w/2}" y="${node.y+24}" text-anchor="middle" font-size="12" fill="#111827">${esc(row.process)}</text>`+
        `<text x="${node.x+10}" y="${node.y+46}" font-size="9" fill="#64748b">S ${es}</text>`+
        `<text x="${node.x+node.w-10}" y="${node.y+46}" text-anchor="end" font-size="9" fill="#64748b">E ${ef}</text>`;
    }
    let svg=`<div style="margin-bottom:20px"><div style="font-weight:600;margin:0 0 8px 0;color:#0f172a">Parent: ${esc(parent)}</div><svg width="100%" height="${Math.max(maxY+50,180)}" viewBox="0 0 ${Math.max(maxX+50,700)} ${Math.max(maxY+50,180)}" preserveAspectRatio="xMinYMin meet"><defs><marker id="nestedArrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,6 L9,3 z" fill="#6b7c93" /></marker></defs><rect x="0" y="0" width="${Math.max(maxX+50,700)}" height="${Math.max(maxY+50,180)}" fill="#fcfdff"/>`;
    edges.forEach(e=>{ svg+=drawOrthoPath(routeEdge(e.from,e.to),'#6b7c93',1.7,'nestedArrow'); });
    Object.entries(positions).forEach(([proc,node])=>{ svg+=cardMarkup(node,scheduleMap[proc]); });
    svg+=`</svg></div>`;

    // recurse deeper
    rows.forEach(r=>{ svg += renderOne(r.process); });
    return svg;
  }

  const topParents=(byParent['']||[]).map(r=>r.process).filter(p=>(byParent[p]||[]).length>0);
  if(!topParents.length){
    host.innerHTML='<div class="small">No nested process networks in the current input.</div>';
    return;
  }
  host.innerHTML=topParents.map(p=>renderOne(p)).join('');
}
