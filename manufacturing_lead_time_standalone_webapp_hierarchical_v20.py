...
async function runCalculation(){
  ...
  try{
    ...
    document.getElementById('summary').innerHTML=`<div class="status-warn"><b>Network error</b><div>${esc(String(err))}</div></div>`;
    return;
  }
  ...
  try{
    ...
    document.getElementById('summary').innerHTML=`<div class="status-warn"><b>Response read error</b><div>${esc(String(err))}</div></div>`;
    return;
  }
  ...
  if(rawText){
    try{
      ...
      document.getElementById('summary').innerHTML=`<div class="status-warn"><b>Backend returned non-JSON</b><div>HTTP ${res.status}</div><div style="margin-top:6px">${esc(rawText.slice(0,500))}</div></div>`;
      return;
    }
  }

  if(!res.ok){
    const detail=(data && data.detail)?String(data.detail):`Request failed with HTTP ${res.status}`;
    document.getElementById('summary').innerHTML=`<div class="status-warn"><b>Calculation error</b><div style="margin-top:6px">${esc(detail)}</div></div>`;
    return;
  }
  ...
  try{
    ...
    document.getElementById('summary').innerHTML=`<div class="status-warn"><b>Frontend rendering error</b><div style="margin-top:6px">${esc(String(err))}</div></div>`;
  }
}
