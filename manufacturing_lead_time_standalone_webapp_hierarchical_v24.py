...
function renderNetwork(data,shouldFit=true){
  ...
  function computeLevels(rows, edges){
    ...
    while(changed){
      changed=false;
      edges.forEach(e=>{
        const fromL=levels[e.from]??0, toL=levels[e.to]??0;
        if(toL<=fromL){levels[e.to]=fromL+1; changed=true;}
      });
    }
  ...
  const positions={}, containers={}, visibleNodeIds=new Set(), reservedRects=[];
  const PARENT_HEADER_H = 34;
  ...
  function addReserved(rect){ reservedRects.push({...rect}); }
  ...
  function layoutSubgraph(parentKey, originX, originY, depth){
    ...
    const cardW=depth===0?154:132, cardH=depth===0?62:54;
    const colGap=120, laneGap=52, innerPad=22, headerGap=16, containerPad=18;
    let maxRight=originX, maxBottom=originY;

    rows.forEach(row=>{
      const level=levels[row.process]||0;
      const lane=lanes[row.process]||0;
      let x=originX+innerPad+level*(cardW+colGap);
      let y=originY+innerPad+lane*(cardH+laneGap);
      let rect={x,y,w:cardW,h:cardH};

      let guard=0;
      while(reservedRects.some(r=>rectsOverlap(rect,r,18)) && guard<100){
        const blockers=reservedRects.filter(r=>rectsOverlap(rect,r,18));
        const shiftX=Math.max(...blockers.map(r=>r.x+r.w+36-rect.x));
        x += Math.max(36, shiftX);
    ...
    rows.forEach(row=>{
      ...
      if(childRows.length){
        const childOriginX=node.x-6;
        const childOriginY=node.y+node.h+headerGap+12;
        const sub=layoutSubgraph(proc, childOriginX, childOriginY, depth+1);
        box.w=Math.max(box.w,(childOriginX-box.x)+sub.width+18);
        box.h=Math.max(box.h,(childOriginY-box.y)+sub.height+18);
      }
      containers[proc]=box
      addReserved(box);
  ...
  const top=layoutSubgraph('',24,24,0);
  networkViewport.contentW=Math.max(top.width+100,1120);
  networkViewport.contentH=Math.max(top.height+100,600);
  ...
  function routeEdge(fromId,toId){
    ...
    const fromParent = fromRow?.refines || '';
    const fromContainer = fromParent ? containers[fromParent] : null;
    ...
    let points=[start];
    if(sameParent && fromParent){
      const contentBox={x:fromContainer.x+8,y:fromContainer.y+PARENT_HEADER_H+4,w:fromContainer.w-16,h:Math.max(0,fromContainer.h-PARENT_HEADER_H-12)};
      const midX=Math.max(contentBox.x+10, Math.min(contentBox.x+contentBox.w-10, (start.x+end.x)/2));
      const midY=Math.max(contentBox.y+10, Math.min(contentBox.y+contentBox.h-10, (start.y+end.y)/2));
      if(start.y!==end.y && start.x!==end.x){
        points.push({x:midX,y:start.y},{x:midX,y:end.y});
      } else if(start.x===end.x){
        const sideX=Math.min(contentBox.x+contentBox.w-10,start.x+18);
        points.push({x:sideX,y:start.y},{x:sideX,y:end.y});
      } else {
        points.push({x:midX,y:start.y},{x:midX,y:end.y});
      }
    } else {
      const obstacles=Object.entries(positions)
        ...
        .map(([,p])=>p)
        .concat(
          Object.entries(containers).map(([,box])=>({
            x:box.x,
            y:box.y+PARENT_HEADER_H,
            w:box.w,
            h:Math.max(0,box.h-PARENT_HEADER_H)
          }))
        );
      ...
      if(end.x >= start.x + 30){
        points.push({x:defaultMidX,y:start.y},{x:defaultMidX,y:end.y});
      } else if(end.y >= start.y + 30){
        points.push({x:start.x,y:defaultMidY},{x:end.x,y:defaultMidY});
      } else {
        points.push({x:start.x+28,y:start.y},{x:start.x+28,y:defaultMidY},{x:end.x-28,y:defaultMidY},{x:end.x-28,y:end.y});
      }
      ...
      for(let pass=0; pass<20; pass++){
        ...
        for(let i=0;i<points.length-1;i++){
          for(const obs of obstacles){
            if(segmentHitsRect(points[i],points[i+1],obs,8)){
              adjusted=true;
              if(points[i].x===points[i+1].x){
                const nx=obs.x+obs.w+18;
                points[i].x=points[i+1].x=nx;
              } else if(points[i].y===points[i+1].y){
                const ny=obs.y+obs.h+18;
    ...
    for(let i=1;i<points.length;i++){
      const p=points[i], q=compact[compact.length-1];
      if(p.x!==q.x || p.y!==q.y) compact.push(p);
    }
    return compact.map((p,idx)=>`${idx===0?'M':'L'} ${p.x} ${p.y}`).join(' ');
  }
