...
function renderNetwork(data){
  ...
  const topLevel=schedule.filter(r=>r.depth===0).sort((a,b)=>a.earliest_start-b.earliest_start||a.process.localeCompare(b.process));
  const byParent={};
  schedule.forEach(r=>{const p=r.refines||'';(byParent[p]||(byParent[p]=[])).push(r);});
  Object.values(byParent).forEach(arr=>arr.sort((a,b)=>a.earliest_start-b.earliest_start||a.process.localeCompare(b.process)));
  const allEdges=graph.edges||[];
  const topNodes=new Set(topLevel.map(r=>r.process));
  const topEdges=allEdges.filter(e=>topNodes.has(e.from)&&topNodes.has(e.to));
  const childEdgesByParent={};
  allEdges.forEach(e=>{
    const fr=scheduleMap[e.from], to=scheduleMap[e.to];
    if(fr && to && fr.refines && fr.refines===to.refines){
      (childEdgesByParent[fr.refines]||(childEdgesByParent[fr.refines]=[])).push(e);
    }
  });

  function computeLevels(nodes, edges){
    const incoming={}, outgoing={};
    nodes.forEach(id=>{incoming[id]=[]; outgoing[id]=[];});
    edges.forEach(e=>{ if(incoming[e.to] && outgoing[e.from]){ incoming[e.to].push(e.from); outgoing[e.from].push(e.to); }});
    const levels={}, q=nodes.filter(id=>incoming[id].length===0).sort();
    ...
    while(q.length){
      ...
      outgoing[id].forEach(next=>{
        ...
        if(incoming[next].length===0) q.push(next);
      });
    }
    nodes.forEach(id=>{ if(levels[id]===undefined) levels[id]=0; });
    return levels;
  }

  function assignRows(nodeRows, edges, levels){
    const byLevel={};
    nodeRows.forEach(r=>{const lvl=levels[r.process]||0;(byLevel[lvl]||(byLevel[lvl]=[])).push(r);});
    const rowOf={}, used={};
    Object.keys(byLevel).map(Number).sort((a,b)=>a-b).forEach(level=>{
      ...
      byLevel[level].forEach(row=>{
        const preds=edges.filter(e=>e.to===row.process).map(e=>e.from).filter(p=>rowOf[p]!==undefined);
        let preferred=preds.length ? Math.round(preds.reduce((s,p)=>s+rowOf[p],0)/preds.length) : 0;
        let candidate=preferred;
        while((used[level]||new Set()).has(candidate)) candidate++;
        (used[level]||(used[level]=new Set())).add(candidate);
        rowOf[row.process]=candidate;
      });
    });
    return rowOf;
  }

  const topLevels=computeLevels(topLevel.map(r=>r.process), topEdges);
  const topRows=assignRows(topLevel, topEdges, topLevels);

  const marginX=40, marginY=36, colW=210, rowH=132;
  const cardW=142, cardH=56, childCardW=112, childCardH=48;
  const nodePos={}, parentBoxes={};
  let maxX=0, maxY=0;

  topLevel.forEach(row=>{
    const proc=row.process;
    const level=topLevels[proc]||0;
    const rowIdx=topRows[proc]||0;
    const x=marginX+level*colW;
    const y=marginY+rowIdx*rowH;
    nodePos[proc]={x,y,w:cardW,h:cardH,depth:0,parent:proc};
    const children=(byParent[proc]||[]).filter(c=>c.process!==proc);
    const expanded=!!expandedParents[proc];
    let boxX=x-18, boxY=y-22, boxW=cardW+36, boxH=cardH+40;

    if(children.length && expanded){
      const childIds=children.map(c=>c.process);
      const childEdges=childEdgesByParent[proc]||[];
      const childLevels=computeLevels(childIds, childEdges);
      const childRows=assignRows(children, childEdges, childLevels);
      const innerX=22, innerY=18, childColW=144, childRowH=76;
      let childMaxX=boxX, childMaxY=boxY;
      children.forEach(child=>{
        const cx=boxX+innerX+(childLevels[child.process]||0)*childColW;
        const cy=y+cardH+34+innerY+(childRows[child.process]||0)*childRowH;
        nodePos[child.process]={x:cx,y:cy,w:childCardW,h:childCardH,depth:child.depth||1,parent:proc};
        childMaxX=Math.max(childMaxX,cx+childCardW);
        childMaxY=Math.max(childMaxY,cy+childCardH);
      });
      boxW=Math.max(cardW+36, childMaxX-boxX+24);
      boxH=Math.max(cardH+40, childMaxY-boxY+24);
    }
    parentBoxes[proc]={x:boxX,y:boxY,w:boxW,h:boxH,expanded,children};
    maxX=Math.max(maxX, boxX+boxW);
    maxY=Math.max(maxY, boxY+boxH);
  });

  const width=Math.max(maxX+50, 1240);
  const height=Math.max(maxY+40, 320);
  ...
  function drawOrtho(start, end, color, widthStroke, marker){
    let d="";
    if(Math.abs(end.y-start.y) < 10){
      ...
      d=`M ${start.x} ${start.y} L ${start.x+24} ${start.y} L ${start.x+24} ${midY} L ${end.x-24} ${midY} L ${end.x-24} ${end.y} L ${end.x} ${end.y}`;
    }
    return `<path d="${d}" fill="none" stroke="${color}" stroke-width="${widthStroke}" marker-end="url(#${marker})" stroke-linejoin="round" stroke-linecap="round" />`;
  }

  function taskCard(p, row, isTop, isDominant){
    ...
    const sw=isDominant?2.8:1.4;
    const headerH=isTop?18:16;
    const titleY=p.y+headerH+10;
    return `
      <rect x="${p.x}" y="${p.y}" width="${p.w}" height="${p.h}" rx="${isTop?10:8}" fill="#ffffff" stroke="${stroke}" stroke-width="${sw}" />
      <line x1="${p.x}" y1="${p.y+headerH}" x2="${p.x+p.w}" y2="${p.y+headerH}" stroke="#cbd5e1" stroke-width="1" />
      <text x="${p.x+8}" y="${p.y+12}" font-size="${isTop?11:10}" fill="#475569">${isTop?'Task':'Subtask'}</text>
      <text x="${p.x+p.w/2}" y="${titleY}" text-anchor="middle" font-size="${isTop?13:12}" fill="#111827">${esc(row.process)}</text>
      <text x="${p.x+8}" y="${p.y+p.h-18}" font-size="${isTop?10:9}" fill="#64748b">ES ${es}</text>
      <text x="${p.x+p.w/2}" y="${p.y+p.h-18}" text-anchor="middle" font-size="${isTop?10:9}" fill="#64748b">Dur ${dur}</text>
      <text x="${p.x+p.w-8}" y="${p.y+p.h-18}" text-anchor="end" font-size="${isTop?10:9}" fill="#64748b">EF ${ef}</text>
    `;
  }
  ...
  // Parent containers
  Object.entries(parentBoxes).forEach(([proc, box])=>{
    const hasChildren=(box.children||[]).length>0;
    svg += `<rect x="${box.x}" y="${box.y}" width="${box.w}" height="${box.h}" rx="18" fill="rgba(148,163,184,0.06)" stroke="#cbd5e1" stroke-width="1.5" />`;
    if(hasChildren){
      const bx=box.x+box.w-26, by=box.y+10;
      svg += `<g onclick="toggleParent('${proc}')" style="cursor:pointer"><rect x="${bx}" y="${by}" width="18" height="18" rx="4" fill="#ffffff" stroke="#94a3b8" stroke-width="1.2" /><text x="${bx+9}" y="${by+13}" text-anchor="middle" font-size="14" fill="#334155">${box.expanded?'-':'+'}</text></g>`;
    }
  });
  ...
  topEdges.forEach(e=>{
    ...
    const end = leftMid(to);
    svg += drawOrtho(start, end, dominantEdges.has(`${e.from}__${e.to}`)?'#dc2626':'#94a3b8', dominantEdges.has(`${e.from}__${e.to}`)?3:1.8, dominantEdges.has(`${e.from}__${e.to}`)?'arrowRed':'arrow');
  });

  // Child edges
  Object.entries(childEdgesByParent).forEach(([parent, edges])=>{
    if(!expandedParents[parent]) return;
    edges.forEach(e=>{
      const from=nodePos[e.from], to=nodePos[e.to];
      if(!from || !to) return;
      const start = (to.x > from.x) ? rightMid(from) : bottomMid(from);
      const end = (to.x > from.x) ? leftMid(to) : topMid(to);
      svg += drawOrtho(start, end, '#94a3b8', 1.5, 'arrow');
    });
  });

  // Task cards
  Object.entries(nodePos).forEach(([id,p])=>{
    const row=scheduleMap[id];
    const isTop=p.depth===0;
    const isDominant=isTop && dominantPath.has(id);
    svg += taskCard(p, row, isTop, isDominant);
  });
