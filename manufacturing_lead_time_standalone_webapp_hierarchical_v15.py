...
let networkViewport={scale:1,tx:0,ty:0,minScale:0.25,maxScale:3.5,contentW:1200,contentH:500};
...
function fitNetwork(){
  ...
  if(!host) return;
  const pad=24;
  const vw=Math.max(host.clientWidth-pad,300), vh=640;
...
function renderNetwork(data,shouldFit=true){
  ...
  function layoutSubgraph(parentKey, rows, edges, originX, originY, depth){
    ...
    const cardW=depth===0?148:126, cardH=depth===0?60:52, colGap=92, laneGap=44, innerPad=20, headerGap=18;
  ...
  networkViewport.contentW=Math.max(top.width+48,900);
  networkViewport.contentH=Math.max(top.height+48,520);
  ...
  function orthoPath(start,end){
    if(end.x >= start.x + 28){
      const midX=(start.x+end.x)/2;
      return `M ${start.x} ${start.y} L ${midX} ${start.y} L ${midX} ${end.y} L ${end.x} ${end.y}`;
    }
    const doglegX=start.x+26;
    const midY=(start.y+end.y)/2;
    return `M ${start.x} ${start.y} L ${doglegX} ${start.y} L ${doglegX} ${midY} L ${end.x-26} ${midY} L ${end.x-26} ${end.y} L ${end.x} ${end.y}`;
  }
  function drawEdge(start,end,color,strokeW,marker){
    return `<path d="${orthoPath(start,end)}" fill="none" stroke="${color}" stroke-width="${strokeW}" marker-end="url(#${marker})" stroke-linecap="round" stroke-linejoin="round" />`;
  }
  ...
  allEdges.forEach(e=>{
    ...
    if(!from || !to) return;
    const sameParent=(scheduleMap[e.from]?.refines||'') === (scheduleMap[e.to]?.refines||'');
    const start=(to.x > from.x + 20)?rightMid(from):bottomMid(from);
    const end=(to.x > from.x + 20)?leftMid(to):topMid(to);
    const isTop=(scheduleMap[e.from]?.depth||0)===0 && (scheduleMap[e.to]?.depth||0)===0;
    const isDom=isTop && dominantEdges.has(`${e.from}__${e.to}`);
    svg+=drawEdge(start,end,isDom?'#dc2626':'#7b8ba1',isDom?3:1.8,isDom?'arrowRed':'arrow');
  });
