...
function renderNetwork(data,shouldFit=true){
  ...
  function computeLevels(rows, edges){
    ...
    // disconnected / mid-graph entry nodes still go before successors
    let changed=true;
    while(changed){
      changed=false;
      edges.forEach(e=>{
  ...
  function layoutSubgraph(parentKey, originX, originY, depth){
    ...
    const cardW=depth===0?152:130, cardH=depth===0?62:54;
    const colGap=110, laneGap=46, innerPad=22, headerGap=18, containerPad=18;
    let maxRight=originX, maxBottom=originY;
    const levelMaxX={}, laneMaxY={};

    rows.forEach(row=>{
      const level=levels[row.process]||0;
      const lane=lanes[row.process]||0;
      const baseX=originX+innerPad+level*(cardW+colGap);
      const baseY=originY+innerPad+lane*(cardH+laneGap);
      const x=Math.max(baseX, (levelMaxX[level]??baseX));
      const y=Math.max(baseY, (laneMaxY[lane]??baseY));
      positions[row.process]={x,y,w:cardW,h:cardH,depth,parentKey};
      visibleNodeIds.add(row.process);
      levelMaxX[level]=x+cardW+24;
      laneMaxY[lane]=y+cardH+18;
      maxRight=Math.max(maxRight,x+cardW);
      maxBottom=Math.max(maxBottom,y+cardH);
    });

    // expand children recursively and reflow only nodes to the right / below
    rows.forEach(row=>{
      ...
      let box={x:node.x-containerPad,y:node.y-containerPad,w:node.w+2*containerPad,h:node.h+2*containerPad,expanded,children:childRows.map(r=>r.process)};
      if(expanded){
        ...
        const shiftX=Math.max(0, box.w-(node.w+2*containerPad))+48;
        const shiftY=Math.max(0, box.h-(node.h+2*containerPad))+28;

        rows.forEach(other=>{
          if(other.process===proc) return;
          const pos=positions[other.process];
          if(!pos) return;
          const isRight=pos.x>=node.x+node.w*0.5;
          const overlapsVert=!(pos.y>box.y+box.h+12 || pos.y+pos.h<box.y-12);
          if(isRight && overlapsVert){ pos.x += shiftX; }
          const isBelow=pos.y>node.y;
          const overlapsHoriz=!(pos.x>box.x+box.w+12 || pos.x+pos.w<box.x-12);
          if(isBelow && overlapsHoriz){ pos.y += shiftY; }
        });
  ...
  networkViewport.contentW=Math.max(top.width+90,1080);
  networkViewport.contentH=Math.max(top.height+90,580);
  ...
  function buildOrthoRoute(fromId,toId){
    ...
    if(start.x!==end.x && start.y!==end.y){
      ...
      points=[start,{x:start.x+26,y:start.y},{x:start.x+26,y:midY},{x:end.x-26,y:midY},{x:end.x-26,y:end.y},end];
    } else {
      const midX=(start.x+end.x)/2;
      points=[start,{x:midX,y:start.y},{x:midX,y:end.y},end];
    }

    const obstacleBoxes=Object.entries(positions).filter(([id])=>id!==fromId && id!==toId).map(([,p])=>p)
      .concat(
        Object.entries(containers)
          .filter(([id,box])=>id!==fromId && id!==toId && box.expanded)
          .map(([,box])=>({x:box.x,y:box.y+34,w:box.w,h:Math.max(0,box.h-34)}))
      );
      ...
      for(const box of obstacleBoxes){
        if(segmentHitsBox(points[i],points[i+1],box)){
          if(points[i].x===points[i+1].x){
            const nx=box.x+box.w+16;
            points[i].x=points[i+1].x=nx;
          }else if(points[i].y===points[i+1].y){
            const ny=box.y+box.h+16;
  ...
  function drawEdge(fromId,toId,color,strokeW,marker){
    return `<path d="${buildOrthoRoute(fromId,toId)}" fill="none" stroke="${color}" stroke-width="${strokeW}" marker-end="url(#${marker})" stroke-linecap="round" stroke-linejoin="round" />`;
  }
  ...
  Object.entries(containers).forEach(([proc,box])=>{
    ...
    svg+=`<rect x="${box.x}" y="${box.y}" width="${box.w}" height="${box.h}" rx="18" fill="rgba(148,163,184,0.06)" stroke="#cfd8e3" stroke-width="1.4" /><line x1="${box.x}" y1="${box.y+headerHeight}" x2="${box.x+box.w}" y2="${box.y+headerHeight}" stroke="#d8e2ec" stroke-width="1" />`;
  });
