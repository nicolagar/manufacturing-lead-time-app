...
SAMPLE_ROWS = [
    {"process":"TOP_A","refines":"","predecessor":"NA","successor":"TOP_B","duration":"","workload":"","workers":"","set_up":""},
...
def safe_eval_formula(expression: str, variables: Dict[str, float]) -> float:
    ...
    return float(eval(compile(tree, "<formula>", "eval"), {"__builtins__": {}}, {**funcs, **variables}))

def resolve_numeric_or_formula_duration(row: pd.Series, formula_map: Dict[str, str], extra_columns: List[str]) -> Optional[float]:
    numeric_duration = coerce_number(row["duration_raw"])
    if numeric_duration is not None: return numeric_duration
    token = normalize_text(row["duration_raw"])
...
def compute_level_schedule(level_df: pd.DataFrame, offset: float) -> Tuple[pd.DataFrame, float, nx.DiGraph]:
    ...
    for n in order:
        if list(g.predecessors(n)): continue
        succs = [s for s in g.successors(n) if indeg[s] > 1]
        feasible = [es[s] - duration[n] for s in succs if es[s] - duration[n] >= 0]
        if feasible:
            es[n] = min(feasible); ef[n] = es[n] + duration[n]
    for n in order:
        preds = list(g.predecessors(n))
        if preds: es[n] = max(ef[p] for p in preds)
        ef[n] = es[n] + duration[n]
    for n in order:
        succs = list(g.successors(n)); preds = list(g.predecessors(n))
        if succs or len(preds) != 1: continue
        pred = preds[0]
        if outdeg[pred] > 1:
            es[n] = max(es[n], ef[pred]); ef[n] = es[n] + duration[n]
    for n in order:
        preds = list(g.predecessors(n))
        if preds: es[n] = max(ef[p] for p in preds)
        ef[n] = es[n] + duration[n]
    ...
    for n in reversed(order):
        ...
        lf[n] = min((ls[s] for s in succs), default=lead)
        ls[n] = lf[n] - duration[n]
    recs = []
    for n in order:
        recs.append({"process":n,"earliest_start":es[n]+offset,"earliest_finish":ef[n]+offset,"latest_start":ls[n]+offset,"latest_finish":lf[n]+offset,"total_float":ls[n]-es[n],"critical":abs(ls[n]-es[n])<1e-9,"predecessors":sorted(g.predecessors(n)),"successors":sorted(g.successors(n))})
...
def compute_hierarchical_schedule(df: pd.DataFrame) -> Dict:
    ...
    def resolve_node_duration(process: str, stack: Optional[Set[str]] = None) -> float:
        ...
        if base is not None:
            ...
            raise PertDataError(f"Process '{process}' has empty duration and no sub-level processes refining it.")
...
function renderGantt(data){const schedule=(data.schedule||[]).slice().sort((a,b)=>a.depth-b.depth||a.earliest_start-b.earliest_start||a.process.localeCompare(b.process));const dominantSet=new Set(data.dominant_path||[]);const width=1200,rowHeight=34,labelWidth=220,rightPad=24,topPad=26,chartWidth=width-labelWidth-rightPad,height=topPad+schedule.length*rowHeight+30,finish=Math.max(data.lead_time||1,1),scale=value=>labelWidth+(value/finish)*chartWidth;let svg=`<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">`;const tickCount=Math.min(Math.round(finish)+1,21);for(let i=0;i<tickCount;i++){const tick=(finish/Math.max(tickCount-1,1))*i,x=scale(tick);svg+=`<line x1="${x}" y1="16" x2="${x}" y2="${height-16}" stroke="#e5e7eb" stroke-width="1" /><text x="${x}" y="12" text-anchor="middle" font-size="11" fill="#6b7280">${Math.round(tick)}</text>`;}schedule.forEach((row,idx)=>{const y=topPad+idx*rowHeight,x=scale(row.earliest_start),w=Math.max(scale(row.earliest_finish)-x,6),hue=(idx*37)%360,fill=`hsl(${hue} 70% 65%)`,isDominant=dominantSet.has(row.process)&&row.depth===0,stroke=isDominant?'#dc2626':row.critical?'#111827':'transparent',sw=isDominant?3:row.critical?2:0,label=`${'— '.repeat(row.depth)}${row.process}`;svg+=`<text x="10" y="${y+18}" font-size="12" fill="#111827">${esc(label)}</text><rect x="${x}" y="${y}" width="${w}" height="20" rx="8" fill="${fill}" stroke="${stroke}" stroke-width="${sw}" /><text x="${x+6}" y="${y+14}" font-size="11" fill="#111827">${Math.round(row.duration*1000)/1000}</text>`;});svg+='</svg>';document.getElementById('ganttContainer').innerHTML=svg;}
...
function renderNetwork(data){const graph=data.graph||{nodes:[],edges:[]},dom=new Set((data.critical_edges||[]).map(e=>`${e.from}__${e.to}`)),levels=computeLevels(graph),groups={};graph.nodes.forEach(n=>{const lvl=levels[n.id]||0;(groups[lvl]||(groups[lvl]=[])).push(n.id)});Object.values(groups).forEach(a=>a.sort());const colWidth=180,rowGap=90,nodeW=120,nodeH=46,margin=30,maxLevel=Math.max(0,...Object.keys(groups).map(Number)),maxRows=Math.max(1,...Object.values(groups).map(a=>a.length)),width=margin*2+(maxLevel+1)*colWidth,height=Math.max(220,margin*2+maxRows*rowGap),pos={};Object.keys(groups).map(Number).sort((a,b)=>a-b).forEach(level=>{groups[level].forEach((id,idx)=>pos[id]={x:margin+level*colWidth+20,y:margin+idx*rowGap+20});});let svg=`<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><defs><marker id="arrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,6 L9,3 z" fill="#6b7280" /></marker><marker id="arrowRed" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,6 L9,3 z" fill="#dc2626" /></marker></defs>`;graph.edges.forEach(e=>{const from=pos[e.from],to=pos[e.to];if(!from||!to)return;const x1=from.x+nodeW,y1=from.y+nodeH/2,x2=to.x,y2=to.y+nodeH/2,mx=(x1+x2)/2,d=`M ${x1} ${y1} C ${mx} ${y1}, ${mx} ${y2}, ${x2} ${y2}`,isDom=dom.has(`${e.from}__${e.to}`)&&e.level_parent==='';svg+=`<path d="${d}" fill="none" stroke="${isDom?'#dc2626':'#94a3b8'}" stroke-width="${isDom?3:1.5}" marker-end="url(#${isDom?'arrowRed':'arrow'})" />`;});graph.nodes.forEach((n,idx)=>{const p=pos[n.id];if(!p)return;const hue=(idx*37)%360,fill=`hsl(${hue} 70% 92%)`,stroke=n.depth===0?'#334155':'#64748b';svg+=`<rect x="${p.x}" y="${p.y}" width="${nodeW}" height="${nodeH}" rx="12" fill="${fill}" stroke="${stroke}" stroke-width="1.5" /><text x="${p.x+nodeW/2}" y="${p.y+nodeH/2+4}" text-anchor="middle" font-size="13" fill="#111827">${esc(n.id)}</text>`;});svg+='</svg>';document.getElementById('networkContainer').innerHTML=svg;}
