...
function renderNetwork(data,shouldFit=true){
  ...
  function layoutSubgraph(parentKey, originX, originY, depth){
    ...
    const cardW=depth===0?150:126, cardH=depth===0?60:52;
    const colGap=116, laneGap=48, innerPad=28, headerGap=18, containerPad=20;
  ...
  networkViewport.contentW=Math.max(top.width+110,1400);
  networkViewport.contentH=Math.max(top.height+110,760);
  ...
  function routePoints(points, obstacles, maxPasses=12){
    let pts=points.map(p=>({...p}));
    for(let pass=0; pass<maxPasses; pass++){
      let adjusted=false;
      for(let i=0;i<pts.length-1;i++){
        for(const obs of obstacles){
          if(segmentHitsRect(pts[i],pts[i+1],obs,8)){
            adjusted=true;
  ...
  function routeEdge(fromId,toId){
    ...
    if(sameParent && parentContainer){
      ...
      const midX=Math.max(content.x+14,Math.min(content.x+content.w-14,(start.x+end.x)/2));
      if(start.x!==end.x && start.y!==end.y){
        points.push({x:midX,y:start.y},{x:midX,y:end.y});
      } else if(start.x===end.x){
        const sideX=Math.min(content.x+content.w-14,start.x+22);
      ...
      const internalObstacles=Object.entries(positions)
        ...
        .map(([,p])=>p);
      points.push(end);
      const routed=routePoints(points, internalObstacles, 10);
      return routed.map((p,idx)=>`${idx===0?'M':'L'} ${p.x} ${p.y}`).join(' ');
    }

    const externalObstacles=Object.entries(positions)
      .filter(([id])=>id!==fromId && id!==toId)
      .map(([,p])=>p);

    const contentObstacles=Object.entries(containers)
      .map(([,box])=>({x:box.x+8,y:box.y+PARENT_HEADER_H,w:box.w-16,h:Math.max(0,box.h-PARENT_HEADER_H-10)}));

    const obstacles=externalObstacles.concat(contentObstacles);
    ...
    if(end.x >= start.x + 36){
      ...
      points.push({x:dogX1,y:start.y},{x:dogX1,y:midY},{x:dogX2,y:midY},{x:dogX2,y:end.y});
    }
    points.push(end);
    const routed=routePoints(points, obstacles, 12);
    return routed.map((p,idx)=>`${idx===0?'M':'L'} ${p.x} ${p.y}`).join(' ');
  }
  ...
  function taskCard(node,row,isTop,isDominant){
    ...
    const titleY=node.y+node.h/2+4;
    const metaVisible=networkViewport.scale>=0.85;
    return `<rect x="${node.x}" y="${node.y}" width="${node.w}" height="${node.h}" rx="${isTop?8:7}" fill="#ffffff" stroke="${stroke}" stroke-width="${strokeW}" />` +
           `<text x="${node.x+node.w/2}" y="${titleY}" text-anchor="middle" font-size="${isTop?13:12}" fill="#111827">${esc(row.process)}</text>` +
           `${metaVisible?`<text x="${node.x+node.w/2}" y="${node.y+node.h-8}" text-anchor="middle" font-size="9" fill="#64748b">${dur}</text>`:''}`;
  }
  ...
  Object.entries(containers).forEach(([proc,box])=>{
    ...
    svg+=`<rect x="${box.x}" y="${box.y}" width="${box.w}" height="${box.h}" rx="16" fill="rgba(148,163,184,0.05)" stroke="#d4dde7" stroke-width="1.2" />`;
  });

  const dominantPath=new Set(data.dominant_path||[]);
  const dominantEdges=new Set((data.critical_edges||[]).map(e=>`${e.from}__${e.to}`));

  // Draw edges after containers and before nodes so they stay readable but not over text.
  allEdges.forEach(e=>{
    ...
    svg+=drawEdge(e.from,e.to,isDom?'#dc2626':'#6b7c93',isDom?2.8:1.9,isDom?'arrowRed':'arrow');
  });
